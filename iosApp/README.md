...
> 最新范围：先完成传图流程与体验；照片效果、会员、遥控/监看及相关录像暂缓。Windows传图范围重新粗估约80%；GPS也已明确暂缓，不计本轮门槛及比例。原片/RAW预览、EXIF及已有视频文件传输继续，Apple实际编译/真机验收单列。

## 当前实现检查点

第58批已写自动新文件共享去重/入队边界，第59批真实AP/STA已接有界事件记录；中间事件消费、首次扫描基线与自动开关仍待接。Windows实跑275项Python及结构/原UI守卫通过；960项Kotlin/Android（643+317）为第58批证据，本批未改Android/shared，不重复Gradle。38个App Swift、280项XCTest及17项Native预览/位图待Mac编译运行。完整任务及限制以[iOS实现任务清单](../docs/技术调研/iOS实现任务清单.md)顶部为准；本轮约80%，没有达到100%。

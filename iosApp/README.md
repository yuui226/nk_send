...
> 2026-09-08：W01—W50全部WIN-DONE：50/50，Windows本轮传图范围100%（历史刻度100%）。W21—W30已推送8594187，W31—W40已提交f993e8e，W41—W50本次最终提交；实际HEAD/是否推送以git状态为准。1085项Kotlin/Android（768+317）、366项Python、3213个AndroidX EXIF金样、common metadata、Android Debug/Release完整源码编译、Debug Lint与原UI守卫通过；三套依赖快照0变化。相对8594187无Android宿主/平台/版本/打包改动。449项XCTest和17项Native图片样本未运行；Mac仍0/12，下一步M01/M02。Windows实现和检查结束，不继续加功能、不称Swift已编译或可发布；Mac发现问题回开对应W项。 当前只读主表与最终交接包的最新结论，以下旧阶段描述仅为历史。
...
> 2026-09-06：用户暂缓iOS两处照片效果（传输时应用效果、本地效果编辑/成片导出），以后补做；原片预览/EXIF/保存继续。当前本轮Windows范围粗估约70%，此前91%估计偏高；历史批次百分比不代表当前完整功能进度。

> 2026-09-12：照片效果 Windows 侧 iOS 填充已恢复。当前分支已具备共享滤镜目录/分类面板、长按拨轮入口、PHPicker 多选、横向预览、两路并发、shared 内核 JPEG 导出、PhotoKit/Files/分享承载、失败项重试和结果状态；正式页面导航、两处宿主接线及 Apple 编译/真机验证仍待完成。上条暂缓说明保留为历史记录。

> 同日追加：iOS会员及购买/支付/权益恢复也暂缓，苹果端后续单独设计；本轮不移植Android微信支付入口，不改Android/shared既有会员策略。当前范围以实现清单顶部为准，先前约70%是仅暂缓效果时的估计。

> 最新范围：先完成传图流程与体验；照片效果、会员、遥控/监看及相关录像暂缓。Windows传图范围改按剩余任务表计分；GPS也已明确暂缓，不计本轮门槛及比例。原片/RAW预览、EXIF及已有视频文件传输继续，Apple实际编译/真机验收单列。
...
完整实现账本见[任务清单](../docs/技术调研/iOS实现任务清单.md)，最新检查与当前范围以上文为准。Apple源码尚未首次编译，不将Windows测试结果记作iOS可运行。

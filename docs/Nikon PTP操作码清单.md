...
| 0x9424 | **StartTracking** | x, y | 在指定 LV 坐标开始主体追踪对焦 | 🔥 |
...
> 是未合焦。不要在等待期间重复发送 `AfDrive`。`x/y` 必须按增强帧头
> `+28/+30` 给出的显示坐标网格换算，不能直接假定为当前解码 JPEG 的宽高。

> **Z 30 实机探测确认（2026-08-08）**：主体追踪不复用上面的普通 AF 时序，正确调用为
> `StartTracking(x,y)`。无参 `StartTracking()` 返回 `0x2006`；带坐标调用返回 `0x2001`，
> 随后增强帧开始携带选中 AF 框，`EndTracking()` 返回 `0x2001`。追踪坐标使用增强帧头
> `+16/+18` 的完整画面坐标系；普通 `ChangeAfArea` 才使用 `+28/+30` 的 AF 网格。
> `StartTracking` 只建立追踪目标；成功后等待约 80ms，再发送一次 `AfDrive` 并轮询
> `DeviceReady`，才会真正驱动镜头对焦。追踪中的下一次画面点击只执行 `EndTracking`。
...
| 操作码 | 理由 |
|---|---|
| **0x9424/0x9425 StartTracking/EndTracking** | 主体追踪对焦；Z 30 fw 1.20 的 DeviceInfo 已确认同时广告这两个操作码，并非仅限 Z8/Z9。 |

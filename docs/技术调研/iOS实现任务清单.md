...
- 用户当前优先目标：先把iOS传图的流程和体验完整复刻。阶段进度仅衡量本轮传图版在Windows能完成的实现/检查，不代表Apple编译或真机通过；当前重新粗估约80%，此前91%过高、约70%为前一范围估计，均不继续套用。

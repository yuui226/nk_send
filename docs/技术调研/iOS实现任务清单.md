...
| IOS-B02 | WRITTEN | Mac待验 | XCTest target/Scheme已有215个网络/会话/流式传输/目录/队列/保存/预览/Files/定位/蓝牙/元数据/滤镜/Compose控制器场景待执行；Mac脚本串行执行Native commonTest/XCTest/Release编译并保留日志与xcresult |
...
| IOS-T03 | NEXT | Mac待验/真机待验 | 沙盒唯一临时文件、共享同名候选/日期目录、fsync/关闭后move不覆盖、SHA256、失败只清理本次临时文件已写；已有沙盒原片索引按共享文件名/大小/根与日期桶识别，完成后增量记录；Files provider已写完整原片的独占临时副本/逐块复制/源与回读SHA256/不覆盖命名发布（手动Debug入口），provider已复用原扫描/增量索引并绑定授权快照；自动队列目标、共享页面索引/读取和断点保留仍待接 |
...
| IOS-S01 | NEXT | Mac待验/真机待验 | 沙盒及Files目录选择/最小书签/过期刷新/作用域平衡/撤权保留已写；ProviderOriginalStore已接授权内NSFileCoordinator双URL协调、64KiB复制与回读、取消/同名/校验/链接边界及按shared日期命名手动验收；已写同一授权绑定的provider元数据扫描/增量索引、改授权拒绝旧对象、缓存核对实际根路径与重定位重扫；下载仍在沙盒，自动写入/共享页面已有文件绑定、目标预览读取和队列目录变更编排仍须接入 |

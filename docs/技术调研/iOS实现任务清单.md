...
- 本轮剩余重点：完整连接/历史相机/正式共享工作区与设置；按原网格的单张/日期/连拍组入队及浏览预览交互；新增/删除事件与自动入队；STA-direct及媒体/EXIF兼容边界；取消/断线/切网/后台安全停止和恢复；原片保存/分享/目录错误的完整体验及针对性回归。非会员的隐私/权限/工程打包事项继续，Mac实际验收仍单列。

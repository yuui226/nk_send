...
| IOS-B02 | WRITTEN | Mac待验 | XCTest target/Scheme已有234个网络/会话/流式传输/目录/队列/保存/预览/Files/定位/蓝牙/元数据/滤镜/Compose控制器场景待执行；Mac脚本串行执行Native commonTest/XCTest/Release编译并保留日志与xcresult |
...
| IOS-T03 | NEXT | Mac待验/真机待验 | 沙盒唯一临时文件、共享同名候选/日期目录、fsync/关闭后move不覆盖、SHA256、失败只清理本次临时文件已写；已有沙盒原片索引按共享文件名/大小/根与日期桶识别，完成后增量记录；Files provider已写完整原片的独占临时副本/逐块复制/源与回读SHA256/不覆盖命名发布（手动Debug入口），provider已复用原扫描/增量索引并绑定授权快照；provider已有原片/RAW/EXIF读取已复用唯一Apple读取器；索引与普通/RAW/EXIF读取已接单页固定来源；自动队列目标、日期偏好和断点保留仍待接 |

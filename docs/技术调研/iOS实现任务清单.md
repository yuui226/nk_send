...
| IOS-B02 | WRITTEN | Mac待验 | XCTest target/Scheme已有185个网络/会话/流式传输/目录/队列/保存/预览/Files/定位/蓝牙/元数据/滤镜/Compose控制器场景待执行；Mac脚本串行执行Native commonTest/XCTest/Release编译并保留日志与xcresult |
...
| IOS-U06 | NEXT | 双端待验 | 原目录/照片列表/外观三卡及SettingsCard/Divider/SectionLabel/BooleanWheel已共享且Android调用，旧GPS紧凑尺寸引用同一shared值；iOS照片列表设置已接原拨轮/列数/连拍/点击行为并保存恢复，三语原文及两行标签已对照。完整设置外壳/帮助/页脚仍待迁入；目录和外观的Native实际绑定未完成，不展示空操作。剩余设置：目录/按日期整理/自动传输/延后开始、列数/连拍/点击行为、主题/语言/按钮样式/触感/常亮、帮助/反馈/版本/源码入口 |

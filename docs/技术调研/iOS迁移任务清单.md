...
- 下一项：继续[iOS实现任务清单](./iOS实现任务清单.md)；共享基础UI/材质/控件/触感和完整队列正文已接，iOS真实原片队列已接共享正文及主线程结果回传；622项shared+317项app测试通过；204个XCTest与17项iOS预览/位图测试待Mac。Windows可做工作粗估89%（100%=Win可做全部完成，不等于iOS验收）；共享连接控件/队列操作按钮已接，完整原缩略图网格已共享并由Android使用，iOS真实目录/缩略图/整组入队已接同一网格；完整原筛选/日期编辑与弹层已共享，Native日历/文案已备；真实沙盒原片索引/增量更新及原网格已保存徽标已接；真实组合筛选与原未传输退场已接；浏览偏好已保存恢复；真实机身隔离缩略图磁盘缓存/完整目录清理已接；完整扫描后连接级后台填充/日期优先/执行态让路已接；原单图/缩放/旋转已共享并接Native真实图探针；预览分页/连拍/来源快照与意图规则已共享；原预览单页/FHD渐显/连拍堆叠/EXIF信息条和操作按钮已共享并由Android调用；原直方图统计/绘制及开关已共享；完整分页/FHD/EXIF/预取取消/连拍返回/入队飞行已共享并由Android使用；Native真实FHD独立读取/取消/前台令牌及1920原方向解码已写；可选真实异步入队确认/取消/部分接受接点已写，Android同步默认保持；本地普通原片原尺寸/离线读取与来源冻结已写；Native真实位图/网格预览共用缓存及只读磁盘缩略图已写；RAW索引纯解析已提为唯一shared实现，Android原入口委托，Native本地RAW已写安全分段读取/原像素选图/统一取消，原预览EXIF语义已共享且Android委托，Native值载体/数字适配已写，Native本地EXIF安全随机读取/实际属性已写，939项测试及200项脚本检查通过，另有3,197组真实AndroidX数值字节对照PASS；原RATIONAL覆盖已接并锁定0.05EV显隐；多段EXIF/目录别名已补；相机读头命令/截断头数值输入已写；跨重连EXIF正负缓存与远程/本地同键读取已接；预览整段交互优先窗口/下一传输分块让路/取消与关闭释放已写；Native完整预览源/三语原文适配及共享本地路由已写，旋转/直方图偏好已接同一存储及模型；视频信息格式/冻结快照/偏好控件/返回定位已接真实目录入口及共享完整overlay（Apple待验）；原目录/照片列表/外观三设置卡及四控件已共享、Android接入；Native照片列表设置/点击行为已接原拨轮及持久存储。Native外观五项已接独立应用偏好、原主题/纹理/跨页语言及前台常亮生命周期（Mac待验），本批Android源码和打包脚本未改。Files provider的完整原片协调发布/校验/取消基础及手动根目录/日期桶验收入口已写（第47批，Apple待验）；自动队列目标和索引仍未接。继续完整设置外壳/目录provider实际绑定、EXIF边界/扫描批次填充/完整工作区及其它正式页面，不提前收口
- Mac 最近检查点：`M01/M02`；Windows 静态检查不替代 Apple 编译/真机验收

状态只使用：`DONE`、`NEXT`、`TODO`、`MAC`、`BLOCKED`。

- 第47批为Apple独占源码，200项脚本/结构/原UI守卫本批通过；939项Kotlin/Android和Lint沿用第46批，没有重新执行Gradle或打包。

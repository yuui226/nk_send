...
- 下一项：继续[iOS实现任务清单](./iOS实现任务清单.md)；共享基础UI/材质/控件/触感和完整队列正文已接，iOS真实原片队列已接共享正文及主线程结果回传；507项shared+307项app测试通过；128个XCTest与5项iOS位图测试待Mac。Windows可做工作粗估77%（100%=Win可做全部完成，不等于iOS验收）；共享连接控件/队列操作按钮已接，完整原缩略图网格已共享并由Android使用，iOS真实目录/缩略图/整组入队已接同一网格；完整原筛选/日期编辑与弹层已共享，Native日历/文案已备；真实沙盒原片索引/增量更新及原网格已保存徽标已接；真实组合筛选与原未传输退场已接；浏览偏好已保存恢复；真实机身隔离缩略图磁盘缓存/完整目录清理已接；完整扫描后连接级后台填充/日期优先/执行态让路已接；原单图/缩放/旋转已共享并接Native真实图探针；预览分页/连拍/来源快照与意图规则已共享；原预览单页/FHD渐显/连拍堆叠/EXIF信息条和操作按钮已共享并由Android调用；原直方图统计/绘制及开关已共享；完整分页/FHD/EXIF/预取取消/连拍返回/入队飞行已共享并由Android使用；Native真实FHD独立读取/取消/前台令牌及1920原方向解码已写；继续EXIF/本地原片/真实异步入队和Native完整预览接线/扫描批次填充/其余设置/完整工作区及其它正式页面，不提前收口

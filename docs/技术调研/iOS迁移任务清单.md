...
- 最新阶段目标：用户要求先完整复刻iOS传图流程和体验，遥控/监看及相关录像也暂缓；与效果、会员一并保留后续，不算DONE。本轮Windows传图范围改按剩余任务表计分（初始0/50，总计划刻度80.0%），GPS也已获用户明确暂缓，不计本轮门槛及比例。具体门槛以实现清单顶部为准，原片预览/EXIF/视频文件传输不删减。

- 2026-09-06追加范围调整：iOS会员页面、限额/权益接入、购买支付与恢复也暂缓，后续单独重设计；本轮不移植Android微信支付入口，Android/shared现有会员规则不动。照片效果与会员均不计本轮完成门槛且不算DONE；非会员的隐私/权限/工程打包事项继续。

- 2026-09-06范围调整：用户暂缓iOS传输时照片效果和本地效果编辑/成片导出，留待以后；不删除代码、不算已完成、不影响Android。原片预览/EXIF/保存继续。以实现清单当前范围为准，本轮Windows进度重新粗估约70%，此前91%估计偏高，不再沿用。

- 分支：`research/ios`
- 产品版本：`1.81`（Android `versionCode` / iOS build 均为 `54`）
- 当前阶段：Android 共享化已完成；iOS 实现已开始，完整共享 UI 与 iOS 功能尚未完成
- 下一项：继续[iOS实现任务清单](./iOS实现任务清单.md)顶部传图范围。原共享网格/队列/完整预览及设置部分已接真实Apple适配；目录选择/显式目标恢复/原片复用和安全分享已写。第58批新增共享自动入队边界，第59批真实AP/STA有界事件接收已写；事件消费/首次基线、自动设置和完整连接工作区仍待接。960项Kotlin/Android（643+317）、275项Python与结构/原UI守卫通过；280项XCTest及17项Native预览/位图仍Mac待验。总计划刻度见剩余任务表，不等于Apple验收。

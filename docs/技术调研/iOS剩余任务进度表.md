...
- 已推送检查点：`6ab3302`（W06），此前`3cc239d`（交接）、`7dd094d`（审计）、`8503823`（实现）。W07—W10已提交并推送为e5dbadb；W11—W20已提交并推送为ad101d5；W21—W30已推送8594187，W31—W40本次提交，接续以实际Git状态为准，保留全部未提交工作。
...
- 延续历史刻度96.0%仅兼容旧记录，不能据此推算剩余工作量。优先报主任务完成数、W编号与待验内容。
- Windows的WIN-DONE要求真实代码/接线、适用Windows检查通过、Apple专属样本已写并登记；不等于Swift已编译或真机通过。Mac仍0/12，446项XCTest仅是源码方法数，另有17项Native图片测试待Mac；发现缺陷必须重开对应项。
...
- 最新用户要求：完成40提交后直接继续到50；单agent、串行构建，下一项W41。
...
- **Windows 剩余任务：40 / 50 个主项完成，已得40 / 50分，收尾进度80.0%。** W31—W40加10分；剩余10项继续推进。
- **延续此前口径的总计划进度：96.0%。** 固定公式80 + 20 × 已完成分 / 50；不能按历史百分比推算剩余工作量。
...
- 建表不计分；现已完成W01—W40。Windows的100%仍只表示本轮传图范围在Windows能做的实现和检查完成，不等于iOS已编译、可发布或与Android真机完全等价。
...
7. 本表关联旧ID用于追溯，不改变旧总账“DONE须实际验收”的定义。下一执行项W41；最新要求40提交后直接继续50。

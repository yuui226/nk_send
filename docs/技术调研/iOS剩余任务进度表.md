...
以下按可交付功能分组，修复与验收门槛齐备后才结项。**当前已识别 9 组工作：1 组源码修正完成、8 组未完成；更大范围的总剩余数量仍需审计，不能据此称只剩 8 项。** 详情补入各行，后续若发现新缺口增加记录，不用文档“0”替代证据。
...
| C02 批量状态与失败重试 | 未完成 | 异常漏记失败项、迟到进度可覆盖完成、生成中可换图；需确定性并发/重试样本 |

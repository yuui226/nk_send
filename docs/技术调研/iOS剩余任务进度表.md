...
- 已推送检查点：`6ab3302`（W06），此前`3cc239d`（交接）、`7dd094d`（审计）、`8503823`（实现）。W07—W10已提交并推送为e5dbadb；W11—W20已完成并随本次提交归档，接续以实际Git状态为准，保留后续未提交工作。
...
- 当前主任务W01—W20为WIN-DONE，**20/50主项完成，30项未结项，下一项W21**。已达到最新用户要求的20/50，本轮停止，不自行继续W21。W07—W10已推送；W11—W20随本次提交归档，推送状态以Git核对为准。Apple编译/真机仍待Mac。
- 延续历史刻度88.0%仅兼容旧记录，不能据此推算剩余工作量。优先报主任务完成数、W编号与待验内容。
- Windows的WIN-DONE要求真实代码/接线、适用Windows检查通过、Apple专属样本已写并登记；不等于Swift已编译或真机通过。Mac仍0/12，430项XCTest仅是源码方法数，另有17项Native图片测试待Mac；发现缺陷必须重开对应项。
...
- 最新用户要求：先提交推送已完成的10/50（已完成：e5dbadb），然后做到20/50。当前单agent、串行构建；W11—W20完成后停止，不擅自继续W21。
...
- **Windows 剩余任务：20 / 50 个主项完成，已得20 / 50分，收尾进度40.0%。** 本轮W11—W20加10分；达到本轮目标并停止，未完成30项。
- **延续此前口径的总计划进度：88.0%。** 固定公式80 + 20 × 已完成分 / 50；不能按历史百分比推算剩余工作量。
...
- 建表不计分；现已完成W01—W20。Windows的100%仍只表示本轮传图范围在Windows能做的实现和检查完成，不等于iOS已编译、可发布或与Android真机完全等价。
...
7. 本表关联旧ID用于追溯，不改变旧总账“DONE须实际验收”的定义。下一执行项W21；本轮20/50目标已完成并停止，收到继续指令后再推进。

...
以下按可交付功能分组，修复与验收门槛齐备后才结项。**当前已识别 9 组工作：2 组源码修正完成、7 组未完成；更大范围的总剩余数量仍需审计，不能据此称只剩 7 项。** 这里的源码修正完成不代表 Swift 编译或 XCTest 通过。详情补入各行，后续若发现新缺口增加记录，不用文档“0”替代证据。
...
| C03 系统选图和输入/成片文件所有权 | 未完成 | 选图目标名缺少插值、文件清理无稳定 owner、系统 sheet 生命周期可能提前删除文件 |
| C04 预览与原尺寸输出 | 未完成 | 全尺寸解码后被 4MP 预览门槛拒绝；缓存无参数身份/容量限制；输出方向/色彩/错误清理待核实 |
| C05 分类、收藏、拨轮及动态语言 | 未完成 | Swift 分类/排序重复实现、初始化选择不同步、帮助语言更新无人调用；拖动交互待对照 |

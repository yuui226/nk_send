...
- 已推送检查点：`6ab3302`（W06），此前`3cc239d`（交接）、`7dd094d`（审计）、`8503823`（实现）。W07—W10已提交并推送为e5dbadb；W11—W20已提交并推送为ad101d5；本批W21—W30未提交，接续以实际Git状态为准，保留全部未提交工作。
...
- 当前主任务W01—W30为WIN-DONE，**30/50主项完成，20项未结项，下一项W31**。已达到最新用户要求的30/50，本轮停止，不自行继续W31。W11—W20已推送ad101d5，本批W21—W30尚未提交；保留工作区代码及文档。Apple编译/真机仍待Mac。
- 延续历史刻度92.0%仅兼容旧记录，不能据此推算剩余工作量。优先报主任务完成数、W编号与待验内容。
- Windows的WIN-DONE要求真实代码/接线、适用Windows检查通过、Apple专属样本已写并登记；不等于Swift已编译或真机通过。Mac仍0/12，437项XCTest仅是源码方法数，另有17项Native图片测试待Mac；发现缺陷必须重开对应项。
...
### 本轮W21—W30（基线ad101d5，未提交）

- 本轮达到30/50，下一项W31等待指令。任务描述已按Android真实代码纠正：W21没有独立相机复选框全选模式，实际是单张/日期/连拍组入队；W23不自增相机预览分享/删除按钮；W25显示实际速度/耗时，不自造ETA。50项分母不变，详见[工作区与原片交付验收说明](./iOS工作区与原片交付验收说明.md)。
...
- 最新用户要求：继续到30/50。此前W11—W20已提交推送ad101d5；本轮单agent、串行构建，W21—W30已完成并停止，不擅自继续W31，也不自动提交/推送。
...
- **Windows 剩余任务：30 / 50 个主项完成，已得30 / 50分，收尾进度60.0%。** 本轮W21—W30加10分；达到本轮目标并停止，未完成20项。
- **延续此前口径的总计划进度：92.0%。** 固定公式80 + 20 × 已完成分 / 50；不能按历史百分比推算剩余工作量。
...
- 建表不计分；现已完成W01—W30。Windows的100%仍只表示本轮传图范围在Windows能做的实现和检查完成，不等于iOS已编译、可发布或与Android真机完全等价。
...
7. 本表关联旧ID用于追溯，不改变旧总账“DONE须实际验收”的定义。下一执行项W31；本轮30/50目标已完成并停止，收到继续指令后再推进。

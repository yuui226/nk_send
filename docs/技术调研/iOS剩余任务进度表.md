...
> **接手必读（含 Mac 新会话）**：本表是进度与交接主入口，开始工作前必须同时阅读 [《iOS扩展功能任务表》](./iOS扩展功能任务表.md)。另一份文档列出照片效果、GPS、遥控/监看及公共保障的 E/G/R/X 共 34 项详细任务、依赖和完成条件；不能只看本表的 C01—C09 就判断 iOS 剩余范围。按两表继续，每完成一项同步更新相应状态、验证证据和剩余数量。

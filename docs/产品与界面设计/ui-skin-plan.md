...
2. **程序化纹理**（`ui/theme/SkinTexture.kt`）：不用图片资源，运行时按
   (皮肤, 深浅, 稳定种子) 生成无缝平铺 tile（木纹=整数频率正弦条痕 +
   周期扰动；钛合金=低对比喷砂与拉丝纹），透明度烘焙进像素，
...
换成**钛合金**就是把这些 token 换成冷灰金属色，并叠加圆润弧面与钢印内容；换成**木纹**则使用琥珀/金色调及程序化木纹。
...
| `TITANIUM` | 钛合金 | 冷灰 `#68737A` | 0.98 | 冷白 `#EAF1F4` | 喷砂金属、圆润弧面、钢印内容 |
...
| `skin_titanium` | Titanium | 钛合金 | 鈦合金 |
...
- **纹理精度**：钛合金和木纹都由程序化纹理生成；若将来需要更高频细节，可提高 tile 分辨率，但要权衡首次生成成本。
- **强调色也跟皮肤走**：目前所有皮肤共用同一套 accent 色。若想钛合金配冷蓝强调、木纹配深绿，把 `accentBlue` 等也纳入皮肤差异即可（改动仍在同一批工厂对象内）。
...
```kotlin
// 钛合金颜色与材质实现以 ui/theme/Color.kt、ui/theme/SkinTexture.kt 和
// ui/screen/GlassButton.kt 为准，避免设计存档复制一份容易过期的实现。

...
class TransferService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        try {
            ...
            if (com.ztransfer.BuildConfig.DEBUG) {
                android.util.Log.w("ZTransfer", "startForeground failed: ${e.javaClass.simpleName}: ${e.message}")
            }
            stopSelf()
        }
    ...
    private fun createChannel() {
        ...
        // 不做"已存在则跳过"：createNotificationChannel 对已有渠道会合法地更新名称，
        // 因此切换应用内语言后，渠道名会随下一次传输刷新（用户可改的设置不会被覆盖）。
        val manager = getSystemService(NotificationManager::class.java)
        val channel = NotificationChannel(
            CHANNEL_ID,
            com.ztransfer.AppLocale.wrap(this).getString(R.string.transfer_channel_name),
    ...
    private fun buildNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle(localized.getString(R.string.transfer_notification_title))
            .setContentText(localized.getString(R.string.transfer_notification_text))
    ...
    companion object {
        ...
        fun start(context: Context) {
            ...
            try {
                ...
                if (com.ztransfer.BuildConfig.DEBUG) {
                    android.util.Log.w("ZTransfer", "startForegroundService failed: ${e.javaClass.simpleName}: ${e.message}")
                }
            }
        }

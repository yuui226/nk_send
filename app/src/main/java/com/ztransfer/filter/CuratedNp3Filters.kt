package com.ztransfer.filter

import androidx.annotation.StringRes
import com.ztransfer.R

internal data class CuratedNp3FilterDefinition(
    val sourceSha256: String,
    val fallbackName: String,
    @StringRes val nameResId: Int,
    val contrast: Int = 0,
    val highlights: Int = 0,
    val shadows: Int = 0,
    val whites: Int = 0,
    val blacks: Int = 0,
    val saturation: Int = 0,
    val colorMixerBase64: String,
    val toneCurveBase64: String? = null,
)
/** The 50 hand-picked NP3 presets bundled with the app. */
internal val CURATED_NP3_FILTER_DEFINITIONS = listOf(
    CuratedNp3FilterDefinition(
        sourceSha256 = "0d277cf79b6f91dbb70849a34a61d8fbc3a471838731267c3874d2c50061602d",
        fallbackName = "Forest Verdure",
        nameResId = R.string.photo_filter_builtin_forest_verdure,
        saturation = 75,
        colorMixerBase64 = "snGKgHGGXoh2wUlOgLddTmxYdliAimCA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "b08c70e745d56244c22e9fca35a8aa6abf0671ff906fcb1c7caead60e1c5eeb8",
        fallbackName = "Dusk Ember",
        nameResId = R.string.photo_filter_builtin_dusk_ember,
        contrast = 34,
        highlights = -50,
        shadows = -40,
        blacks = -7,
        saturation = 19,
        colorMixerBase64 = "RICAVICAPYCAtYCAgICAd3ZqpICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "bec5204ff213c929a0d8f4aaf3cbef535d3178f7f106ab879ce3f4c02f593646",
        fallbackName = "Bleached Silver",
        nameResId = R.string.photo_filter_builtin_bleached_silver,
        contrast = 40,
        highlights = -20,
        shadows = -55,
        blacks = -10,
        saturation = -50,
        colorMixerBase64 = "gn5xjH53hH56iHFkgntxdHR2fm+AiHR2",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "76aa1920d0f4569431d541dc02a3559c03aa588a55262a40459d5bb0036e45ef",
        fallbackName = "Blue Hour",
        nameResId = R.string.photo_filter_builtin_blue_hour,
        contrast = 10,
        highlights = -10,
        shadows = -20,
        saturation = 20,
        colorMixerBase64 = "gICAgICAno+KgICAgICAgICAgI+AgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "33c925bdda847ecfd77e2aecb4765fb7a9ded77e67b77477ffb310335ed2ac92",
        fallbackName = "Blue Memory",
        nameResId = R.string.photo_filter_builtin_blue_memory,
        colorMixerBase64 = "fpyAgICAYYCALYCAsF2AeqaAgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "dee50954ceed7af5bc331141da2693485592109f7c857994f30c0112fd2cd95e",
        fallbackName = "British Mono",
        nameResId = R.string.photo_filter_builtin_british_mono,
        contrast = 3,
        highlights = -20,
        shadows = 31,
        whites = -34,
        blacks = -10,
        saturation = -98,
        colorMixerBase64 = "YTtxgGCAgUZwgICAgICAgICAgICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "c58ef768566c295700edf11daf9dec02c237e4ae0be874bbe32403256082fa20",
        fallbackName = "Love Glow",
        nameResId = R.string.photo_filter_builtin_love_glow,
        saturation = 30,
        colorMixerBase64 = "gICAgHuPinmElZWUgICAZoCJgICAioSA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "6dbdc3cad35d785215359e452a0e192cafd3c6693cbb98bc640d76c772837981",
        fallbackName = "Calm Breeze",
        nameResId = R.string.photo_filter_builtin_calm_breeze,
        contrast = 18,
        highlights = 31,
        shadows = -20,
        whites = -27,
        blacks = 5,
        saturation = 7,
        colorMixerBase64 = "gICAgICAfIGAgICAgICAgICAgICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "c3aa853ed066c5dfca2ac2450e8176442026e42b913014d41183fe9bd37dd1cd",
        fallbackName = "Cinema Blue",
        nameResId = R.string.photo_filter_builtin_cinema_blue,
        saturation = -5,
        colorMixerBase64 = "nHKegICAgJ2AgEyAlM6AgOSQgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "6586bf9e061a9db3104da36dab3a659ed255f3227cf2b8b32a897f3ca842d46d",
        fallbackName = "Cinematic Dusk",
        nameResId = R.string.photo_filter_builtin_cinematic_dusk,
        colorMixerBase64 = "gICAgICAgICAgICAgICAgICAgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "45bec08a572f5da2383b87dc33ced39b92a6c66652a7ea9f2e51a91e7ca7adc0",
        fallbackName = "Clear Portrait",
        nameResId = R.string.photo_filter_builtin_clear_portrait,
        saturation = 13,
        colorMixerBase64 = "hoCAhYCQgICIgG6AgICAgICAgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "3dd20e9529471c8c1e5e51e8284e2d7ded0b99d72b700b82eab7bfbb3972e017",
        fallbackName = "Cozy Autumn",
        nameResId = R.string.photo_filter_builtin_cozy_autumn,
        contrast = -15,
        highlights = -48,
        shadows = 48,
        whites = -31,
        blacks = -17,
        saturation = -2,
        colorMixerBase64 = "gIiAgICAgICAgICAgICAgICAgICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "e3b23a2ec3225d3bc85703b6530890b27c8ee62085e7802898bf67b4d4eeeb44",
        fallbackName = "Dawn Hues",
        nameResId = R.string.photo_filter_builtin_dawn_hues,
        contrast = -23,
        highlights = -29,
        shadows = 19,
        whites = 29,
        blacks = -17,
        saturation = 8,
        colorMixerBase64 = "gJSJdpSKhZSKhWx2qFh2gFh2gFiAgFiA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "e4d8bc83e450d25d54db2aaa36b4067d03441fa1440a77829570efee7a3d6504",
        fallbackName = "Darkroom Film",
        nameResId = R.string.photo_filter_builtin_darkroom_film,
        highlights = -50,
        shadows = -57,
        blacks = 8,
        saturation = -5,
        colorMixerBase64 = "gIBqgHZ7e2x2gXZ0g2xtgGxfe3BfgHRi",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "e6f26d84a5ba02fd21efd1e5238808325dfb6aba9e2606e3a55c6068a4594a12",
        fallbackName = "Fern Shade",
        nameResId = R.string.photo_filter_builtin_fern_shade,
        colorMixerBase64 = "gICAgHqAgGt2i2dufICAeIlxgGKAgGKA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "a5842375eec8f432b6992dd0d52b42d279b6fecfc27ce1a29772b46cb5cce0a1",
        fallbackName = "Classic Film",
        nameResId = R.string.photo_filter_builtin_classic_film,
        saturation = 20,
        colorMixerBase64 = "in6OinuPZYWecViAfE6KU2JngFiPlF2F",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "9baef65946f0abc5df742526278ba70f18d0892323e725528c12c273d54e69b5",
        fallbackName = "Golden Dusk",
        nameResId = R.string.photo_filter_builtin_golden_dusk,
        saturation = 12,
        colorMixerBase64 = "gICAgJBtT4eEHHudgICAgH9zgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "34a35f27c7d7f51d97852c9cc2cc57cc68f4543ac0c2f302901e844979347631",
        fallbackName = "Soft Harmony",
        nameResId = R.string.photo_filter_builtin_soft_harmony,
        contrast = 8,
        highlights = -27,
        shadows = 13,
        whites = 9,
        blacks = -10,
        saturation = 10,
        colorMixerBase64 = "tIuQaJB6W4aIYIB7LYBtJoCAe4CAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "f4fd3141b1642e2ec734860d50e1a5cd5c9f52b881d9e33149ec7afa656a6991",
        fallbackName = "Urban Green",
        nameResId = R.string.photo_filter_builtin_urban_green,
        contrast = -12,
        highlights = -25,
        shadows = 26,
        whites = -29,
        blacks = -4,
        saturation = 9,
        colorMixerBase64 = "h4B2d3CAjGyFuXqJfICAfXiAgHOFgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "7c7fdcfe871c66e27166bb366bcde9aa29dc2e8fb803a15bf7ffc8c858f27c66",
        fallbackName = "Matte Blue",
        nameResId = R.string.photo_filter_builtin_matte_blue,
        saturation = -6,
        colorMixerBase64 = "h3GEfXuUgXiJd3SYhXGAdHB8iX2Fg3yF",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "71ca9fe9d7afe6eab33094a521c3135bf4bf6d9ff66b2f2a1797517e17a0843f",
        fallbackName = "Moss Mood",
        nameResId = R.string.photo_filter_builtin_moss_mood,
        contrast = 6,
        highlights = 9,
        shadows = 12,
        whites = 8,
        blacks = -17,
        colorMixerBase64 = "gICAgYiBgJ3R32B9ppmAYliAgICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "cd4001092f8f2d77c7e7f10ecaf79970eb79272fbbef6bd98bec98219e9504a6",
        fallbackName = "Green Shadows",
        nameResId = R.string.photo_filter_builtin_green_shadows,
        contrast = 50,
        highlights = -30,
        shadows = -30,
        whites = -40,
        blacks = 10,
        saturation = -5,
        colorMixerBase64 = "gHZogIB2hahsgIVtgIqAgKNTgI+AgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "1be54af9be2e0403524d16d1c64d02fe1832c0d4fde6ca30e0c83c4d9f32a301",
        fallbackName = "Soft Portrait",
        nameResId = R.string.photo_filter_builtin_soft_portrait,
        colorMixerBase64 = "i3+GiICHgICIhHiAgICAcHmLgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "e427f04afb57c5fa8301a596874d7fb8ac3af434a7c0b0d52c6ff3055ae2b992",
        fallbackName = "Natural Link",
        nameResId = R.string.photo_filter_builtin_natural_link,
        saturation = 18,
        colorMixerBase64 = "gICAgICAgICAgICAgICAdJOAgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "6589fe70d5362bb0e2b5f21c7d82610d07f087b4600fd772efdf2329146ddef6",
        fallbackName = "Cyan Negative",
        nameResId = R.string.photo_filter_builtin_cyan_negative,
        shadows = -70,
        blacks = -15,
        colorMixerBase64 = "lICFioB7dnt+o2JgqJlYdnF7hU6FvHGK",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "c35e458aa2673b8d1968497d5c18a03351f1d4cc1635b0b7d5fad7f54805ad42",
        fallbackName = "Red Cyan",
        nameResId = R.string.photo_filter_builtin_red_cyan,
        whites = 10,
        blacks = -15,
        saturation = 10,
        colorMixerBase64 = "YnF2j3t7ipmFWICFj3aPhXaZZ3uAdoCA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "e5b262787ebfece2b216b5de95a5504cd771a1de57fe681531f3232fa9de4af3",
        fallbackName = "Teal Negative",
        nameResId = R.string.photo_filter_builtin_teal_negative,
        contrast = -25,
        highlights = -10,
        shadows = -30,
        whites = -5,
        saturation = 20,
        colorMixerBase64 = "SXFsnnGUipSPXXueqHGjinFngIVinmd7",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "32184d99df35adeccb63d06d504c4e76a7830acf86710cbd0f1c108f3e3d35ab",
        fallbackName = "Lemon Negative",
        nameResId = R.string.photo_filter_builtin_lemon_negative,
        highlights = -10,
        shadows = -5,
        whites = 10,
        blacks = -15,
        saturation = 10,
        colorMixerBase64 = "cWx7lHaUiqiUWICPmWyAinGKe4CAXXGA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "c750bd30a6b411504251e687aef473fdcc664c9db638633fe9d9eb3ff4a341a3",
        fallbackName = "Amber Negative",
        nameResId = R.string.photo_filter_builtin_amber_negative,
        contrast = -15,
        highlights = -10,
        shadows = -15,
        whites = 10,
        blacks = -15,
        saturation = 10,
        colorMixerBase64 = "lF2Po3uUj62PTnt7lGeFimx7Z3GPcWd7",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "ec02d8e5c7df424e08bab9372750e6ed57a9461e85a62aaafbd92047079af42f",
        fallbackName = "Lime Negative",
        nameResId = R.string.photo_filter_builtin_lime_negative,
        contrast = 5,
        whites = 10,
        blacks = -10,
        saturation = 10,
        colorMixerBase64 = "WGxirYWKireUXZmKgE57j3txcYWFcWd7",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "bd36bef9d4cea13ad0430b0ba39a98567622d1ae8e81b5b381f3855d0bbd0978",
        fallbackName = "Pastel Pink",
        nameResId = R.string.photo_filter_builtin_pastel_pink,
        saturation = 5,
        colorMixerBase64 = "jHZ2gH+AinuAgHaAhYCAe3uAdnZidnZh",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "2a00641ffda460ae5184bef8fd0f34b8d0f1d5ce2d890a6a1de6fbfb56532dce",
        fallbackName = "Rose Vintage",
        nameResId = R.string.photo_filter_builtin_rose_vintage,
        saturation = -5,
        colorMixerBase64 = "gICAj26rgF13iGA4gICAj4OAgICAk3Kb",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "8cb0fba31ff25f7434d7f1c753f5d3e1e1ed41784539770a6ec4adc6bdd0bad3",
        fallbackName = "Setouchi Blue",
        nameResId = R.string.photo_filter_builtin_setouchi_blue,
        highlights = -35,
        whites = 9,
        saturation = -13,
        colorMixerBase64 = "gICAgICAgICAgICAgICAgICAgICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "b2d9b01ed879a45eec531d0228fb02a850ad7b823a2860214b95404ced6da01a",
        fallbackName = "Sky Mist",
        nameResId = R.string.photo_filter_builtin_sky_mist,
        saturation = 5,
        colorMixerBase64 = "gHmAh26rgF13jFI6gICAVodugICAk1+J",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "b93eade0b6468c28d94659a92601752151a6150cb648d5edb6cbfa1eba059754",
        fallbackName = "Soft Film",
        nameResId = R.string.photo_filter_builtin_soft_film,
        contrast = -38,
        highlights = -64,
        shadows = 52,
        whites = -33,
        blacks = -40,
        saturation = -10,
        colorMixerBase64 = "gICAgICAgICAgICAgICAgICAgICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "386409e289a1665487902351685a45eb17d5f283aec88339f7227d2c26917526",
        fallbackName = "Soft Glow",
        nameResId = R.string.photo_filter_builtin_soft_glow,
        saturation = 3,
        colorMixerBase64 = "gIWDgYR2an2AW4xpgICAgICAgICAi4CA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "1b0b37b884574266b7fbb2cf3565a7011da48551a80e1146fdded0ddc11335fb",
        fallbackName = "Cool Sun Kiss",
        nameResId = R.string.photo_filter_builtin_cool_sun_kiss,
        contrast = -10,
        highlights = -76,
        shadows = -79,
        whites = -83,
        blacks = 17,
        colorMixerBase64 = "ioCAZYCAdXCAcmaAhI6AgJ6AgGp0gICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "f7575f53f0b0d86f98dda815ba8d04a81baebfec34444aed2fc3ce33f3ad1739",
        fallbackName = "Warm Sun Kiss",
        nameResId = R.string.photo_filter_builtin_warm_sun_kiss,
        contrast = -10,
        highlights = -76,
        shadows = -79,
        whites = -83,
        blacks = -18,
        colorMixerBase64 = "ioCAZYCAdXCAcmaAhI6AgJ6AgGp0gICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "bd2cf970979bfdf15f125ef46fd0ff233fd576029d41effdeec4708af1e083bd",
        fallbackName = "Sunset Film",
        nameResId = R.string.photo_filter_builtin_sunset_film,
        contrast = 50,
        shadows = 45,
        whites = 5,
        blacks = -20,
        saturation = -25,
        colorMixerBase64 = "gIGPiIiUbICPgICAgDQ2gB8cgICAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "877a7c1d0051c3cf06eef1ebe66e2d33da87bd628d999f68eb9036f20a343e3d",
        fallbackName = "Sunset Glow",
        nameResId = R.string.photo_filter_builtin_sunset_glow,
        colorMixerBase64 = "mYpshaiVbGyeYmxYYmKAYmeUwU6AnliK",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "1b4e7591203a4326dc91835945553aee5df39094c16565467b97628ecec0a33b",
        fallbackName = "Teal and Orange",
        nameResId = R.string.photo_filter_builtin_teal_and_orange,
        contrast = 25,
        highlights = 10,
        shadows = -25,
        whites = 4,
        colorMixerBase64 = "jZmed62AgIiAgICAgMaAU6OUgICArYCA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "4a4665eca8d71848bc09c1279e8e7412e4c16c3076b4d5aa09608cebfba00e33",
        fallbackName = "Gentle Clarity",
        nameResId = R.string.photo_filter_builtin_gentle_clarity,
        saturation = 15,
        colorMixerBase64 = "jnaDjIWNeHuKjXaSgICAbImHkn2EkYCD",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "b125d4cf8ad4ad26580b9e1f1cdde75dd90c878f33aac9bc575b409bf05e7b72",
        fallbackName = "Turquoise Blue",
        nameResId = R.string.photo_filter_builtin_turquoise_blue,
        saturation = 5,
        colorMixerBase64 = "gHt3gHuAinuAgHaAgICAYoCAgGxidnZh",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "a9888e7ba4dafdee202cda8dcb3fedaabc85cafa65f2f888fa7ecc820fbdea6c",
        fallbackName = "Vintage Color",
        nameResId = R.string.photo_filter_builtin_vintage_color,
        saturation = 13,
        colorMixerBase64 = "gICAgICAgICAgG2AdYCAdoCAgICAgICA",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "80d9bd7f210f3f55480b4fbb928763b94c3c26a4364ff20d896ab37d75c1cce3",
        fallbackName = "Vintage Vibe",
        nameResId = R.string.photo_filter_builtin_vintage_vibe,
        contrast = 50,
        highlights = -21,
        shadows = -15,
        whites = 29,
        blacks = -7,
        saturation = 20,
        colorMixerBase64 = "hW9xhHBnbmeAgW2AlzGAfIBqmWeAZ2eA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "980ff2d2de250f347009d61079be1ee889bb3b925e49b0b35c5e9aa143ab14c1",
        fallbackName = "Vital Film",
        nameResId = R.string.photo_filter_builtin_vital_film,
        contrast = -4,
        highlights = 54,
        shadows = -17,
        blacks = -3,
        colorMixerBase64 = "eIh6hYmCc2p31VRLsaVrYqtZfI6Ah491",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "5dcf969ef130443ea2fae42027db15800eb2c58d96b4ce8ca2cf69bda6c26429",
        fallbackName = "Warm Street",
        nameResId = R.string.photo_filter_builtin_warm_street,
        contrast = 36,
        highlights = -51,
        shadows = 64,
        whites = 33,
        blacks = -10,
        saturation = 7,
        colorMixerBase64 = "kJq2ebyXUbuMgaGbu0+Afn2AOFNvgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "3b3a55d016485725f311dbc20eb9417606362186143559c4814ab376469efb81",
        fallbackName = "Warm Lowlight",
        nameResId = R.string.photo_filter_builtin_warm_lowlight,
        contrast = 5,
        highlights = 30,
        shadows = 10,
        whites = 25,
        saturation = 15,
        colorMixerBase64 = "gICAgICAgICAmYCAioCAj4CAioCAgICA",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "c3ed7d80ab4b91f56b32f591454c0298c332cf4eaf8dea2b4b4e415e9153d5a7",
        fallbackName = "Warm Portrait",
        nameResId = R.string.photo_filter_builtin_warm_portrait,
        saturation = 8,
        colorMixerBase64 = "gICAgICAgICAhXaAgICAgICAgICAgICQ",
        toneCurveBase64 = "",
    ),
    CuratedNp3FilterDefinition(
        sourceSha256 = "5039ec776c133ceeab61fbd3f7ec62b8df454ec42817cdba880c4f375d4abc22",
        fallbackName = "Honey Warm",
        nameResId = R.string.photo_filter_builtin_honey_warm,
        contrast = 29,
        highlights = -22,
        shadows = 14,
        whites = 22,
        saturation = 20,
        colorMixerBase64 = "eqWQg4mTUptggICAgICAgICAgICAgICA",
    ),
)

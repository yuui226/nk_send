...
internal fun Modifier.geniePopupLayer(
    ...
    .drawWithCache {
        ...
        onDrawWithContent {
            ...
            val rows = List(GENIE_BANDS + 1) {
                genieRow(p, it.toFloat() / GENIE_BANDS, source, bounds, mouthWidth)
            }
            ...
            try {
                ...
                for (index in 0 until GENIE_BANDS) {
                    val sourceTop = size.height * index / GENIE_BANDS
                    val sourceBottom = size.height * (index + 1) / GENIE_BANDS
                    val top = rows[index]

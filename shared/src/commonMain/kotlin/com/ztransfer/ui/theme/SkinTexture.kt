...
class ButtonTexturePalette internal constructor(
    ...
    internal fun cachedBrushFor(seed: Int): Brush? {
        ...
        val variant = textureFloorMod(mixSeed(seed), variantCount)
...
private val tileCacheLock = TextureCacheLock()
...
private fun cachedButtonTextureTile(
    ...
    dark: Boolean,
    variant: Int,
): ImageBitmap? = tileCacheLock.withLock {
...
private fun periodicValueNoise(
    ...
    fun sample(x: Int, y: Int): Float {
        val wrappedX = textureFloorMod(x, cellsX)
        val wrappedY = textureFloorMod(y, cellsY)
